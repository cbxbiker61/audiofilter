#pragma once
#ifndef AUDIOFILTER_CRC_H
#define AUDIOFILTER_CRC_H
...
public:
  ...
  inline uint32_t crc_init(uint32_t crc) const
  {
    return crc << (32 - power);
  }

  inline uint32_t crc_get(uint32_t crc) const
  {
    return crc >> (32 - power);
  }
  ...
  uint32_t calcBits(uint32_t crc, const uint8_t *data, size_t start_bit, size_t bits) const;
  uint32_t addBits(uint32_t crc, uint32_t data, size_t bits) const;
};
